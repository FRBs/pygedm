...
@app.callback(
    args=dict(
        ...
        method=tpl.new_dropdown(
            ["DM (pc/cm3) to Distance", "Distance (kpc) to DM"],
            label="Method",
            kind=dl.State,
        ),
        dmord=tpl.new_textbox(10, label=None, kind=dl.State),
        nu=tpl.new_textbox(1.0, label="Frequency (GHz)", kind=dl.State),
        coords=tpl.new_dropdown(
            ["Galactic (gl, gb)", "Celestial (RA, DEC)"],
            label="Coordinates",
            kind=dl.State,
        ),
...
def callback(model, method, dmord, nu, coords, x0, x1, go, tab):
    ...
    notes = html.Div([
        ...
        html.P([
            ...
            html.A(
                "ADS",
                href="https://ui.adsabs.harvard.edu/abs/2021arXiv210615816P/abstract",
            ),
            " | ",
        ...
        html.P(
            "A New Electron-density Model for Estimation of Pulsar and FRB Distances, ApJ, Volume 888, Issue 2, id.105, Colume 835, id.29"
        ),
        html.P([
            "Web app hosted by ",
