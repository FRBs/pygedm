...
def TAUISS(d, sm, nu):
    ...
    """
    tauiss = 1.0 * (sm / 292.0) ** 1.2 * d * nu ** (-4.4)
    return tauiss


def TRANSITION_FREQUENCY(d_eff: float, sm: float, xi: float = None) -> float:
    ...
    Args:
        ...
        sm (float): Scattering measure (kpc m^{-20/3})
        xi (float): Fresnel scale scaling factor (~1). If not supplied,
                    sqrt(2 pi) is used as preferred definition.

    Returns:
        nu (float): Transition frequency in GHz
    """
    xi = np.sqrt(np.pi * 2) if xi is None else xi
    nu = 318.0 * xi ^ (10 / 17) * sm ** (6 / 17) * d_eff ** (5 / 17)
    return nu
